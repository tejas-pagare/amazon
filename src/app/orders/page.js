...
export default function OrdersPage() {
  ...
  return (
    <div className="bg-white min-h-screen w-full">
      <main className="w-full max-w-5xl mx-auto px-6 py-12 bg-white">
      {/* Header & Search */}
      <div className="mb-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h1 className="text-2xl font-headline font-black tracking-tighter mb-1">Your Orders</h1>
            <p className="text-sm text-on-surface-variant">{orders.length} orders placed in total</p>
          </div>
      ...
      <div className="flex items-center gap-2 mb-10 overflow-x-auto pb-2 no-scrollbar">
        <button className="px-6 py-2 rounded-full bg-secondary-container text-on-secondary-container font-bold shadow-sm text-sm whitespace-nowrap transition-transform active:scale-95 cursor-pointer">All Orders</button>
      </div>

      {/* Order List */}
      <div className="space-y-8">
        {orders.length === 0 ? (
          <div className="bg-surface-container-lowest rounded-lg p-12 text-center text-on-surface-variant border border-outline-variant/30 flex flex-col items-center">
            <span className="material-symbols-outlined text-4xl mb-4 text-outline/50">inbox</span>
            <p>You haven't placed any orders yet.</p>
            <Link href="/" className="mt-4 text-[#2162a1] font-bold hover:underline">Start shopping</Link>
          </div>
        ) : (
          orders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-surface-container-lowest rounded-lg overflow-hidden shadow-sm transition-transform hover:scale-[1.01] duration-300 border border-outline-variant/30">
                <div className="bg-surface-container-high px-6 py-4 flex flex-wrap justify-between items-center gap-4 text-sm border-b border-outline-variant/30">
                  <div className="flex gap-10">
                    <div>
                      <p className="text-[9px] font-label uppercase tracking-wider text-on-surface-variant mb-1">Order Placed</p>
                      <p className="text-xs font-bold">{dateStr}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-label uppercase tracking-wider text-on-surface-variant mb-1">Total</p>
                      <p className="text-xs font-bold">₹{order.total}</p>
                    </div>
                    <div className="hidden sm:block">
                      <p className="text-[9px] font-label uppercase tracking-wider text-on-surface-variant mb-1">Status</p>
                      <p className="text-xs font-bold uppercase tracking-widest text-[#2162a1]">{order.status}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[9px] font-label uppercase tracking-wider text-on-surface-variant mb-1">Order # {order.id.toString().padStart(6, '0')}</p>
                    <div className="flex gap-3 text-[#2162a1] font-bold justify-end text-xs">
                        ...
                        <div className="flex-1 flex gap-6">
                          ...
                          <div className="flex-1">
                            <h3 className="text-base font-headline font-bold text-on-surface mb-1">
                              <Link href={`/product/${item.product_id}`} className="hover:text-[#2162a1] transition-colors">{item.name}</Link>
                            </h3>
                            <p className="text-xs text-on-surface-variant mb-2">Qty: {item.quantity} • ₹{item.unit_price}</p>
                            <div className="flex flex-wrap gap-3 mt-4">
                              <Link href={`/product/${item.product_id}`} className="bg-secondary-container text-on-secondary-container px-6 py-1.5 rounded-full font-bold text-xs hover:opacity-90 transition-all active:scale-95 shadow-sm inline-block cursor-pointer">Buy it again</Link>
                            </div>
                          </div>
                        </div>
                      </div>
